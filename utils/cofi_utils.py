...
def inspect_model_layers(model):
    for name, module in model.named_modules():
        ...
        if hasattr(module, 'adapter'):
            print(f"{name} adapter layers:")
            print(f"proj_down: {module.adapter.proj_down.weight.shape}")
            print(f"proj_down bias: {module.adapter.proj_down.bias.shape}")
            print(f"proj_up: {module.adapter.proj_up.weight.shape}")
            print(f"proj_up bias: {module.adapter.proj_up.bias.shape}")
...
def prune_model_with_z(zs, model):
    ...
    if "hidden_z" in zs:
        ...
        for layer in range(0, 12):
            if bert.encoder.layer[layer].attention.self.query is not None:
                # print("Layer: ", layer)
                # print("Query before pruning:", bert.encoder.layer[layer].attention.self.query)
                ...
                # print("Query after pruning:", bert.encoder.layer[layer].attention.self.query)
                query_weights = bert.encoder.layer[layer].attention.self.query.weight.detach().numpy()
                # print("Query weights matrix shape:", query_weights.shape)
                # print(query_weights)
            if bert.encoder.layer[layer].attention.self.value is not None:
                # print("Layer: ", layer)
                # print("Value before pruning:", bert.encoder.layer[layer].attention.self.value)
