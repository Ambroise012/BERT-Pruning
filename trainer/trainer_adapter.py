...
from models.adapter import AdapterModule
...
class CoFiTrainer(Trainer):
    ...
    #     optimizer = torch.optim.AdamW(optimizer_grouped_parameters, lr=self.args.learning_rate, eps=self.hparams.adam_epsilon)
    ...
    def configure_optimizers_and_scheduler(self, num_training_steps: int, build_l0_optimizer: bool=True):
        def log_params(param_groups, des):
            for i, grouped_parameters in enumerate(param_groups):
                logger.info(
                    f"{des}, number of params: {sum(p.nelement() for p in grouped_parameters['params'])}, weight_decay: {grouped_parameters['weight_decay']}, lr: {grouped_parameters['lr']}")
        ...
        # Add parameters for the main model if strategy is not set or additional parameters for all strategies
        ...
        log_params(optimizer_grouped_parameters, "main params")
        ...
        # Additional optimizers for l0 module if required
        l0_optimizer = None
        ...
        if build_l0_optimizer and self.l0_module is not None:
            ...
            lagrangian_optimizer = AdamW(lagrangian_params,
                                        betas=(self.args.adam_beta1, self.args.adam_beta2),
                                        eps=self.args.adam_epsilon)
        # Setting warmup steps as a percentage of total training steps
        warmup_steps = int(0.1 * num_training_steps)  # 10% of total steps
        ...
        # Preparing the scheduler dict for the optimizer
        scheduler = {"scheduler": lr_scheduler, "interval": "step", "frequency": 1} if lr_scheduler else None

        optimizers = [optimizer]
        if l0_optimizer:
            optimizers.append(l0_optimizer)
        if lagrangian_optimizer:
            optimizers.append(lagrangian_optimizer)
    ...
    def save_model(self, output_dir: Optional[str] = None):
        ...
        if self.l0_module is None:
            print("l0_module is not initialized")
            return
