...
def save_metric_to_csv(metrics, task_name, model_size, sparsity):
    ...
    with open(file_name, mode='a', newline='') as file:
        ...
        if not file_exists:
            headers = ["task name", "model size", "sparsity"] + list(metrics.keys())
            writer.writerow(headers)
        
        # Write the metrics on a single line
        row = [task_name, model_size, sparsity] + [metrics[key] for key in metrics]
        writer.writerow(row)
...
if __name__ == '__main__':
    ...
    for key in metrics:
        if metrics[key] is not None:
            ...
            print(f"{key}: None")
    print()
    save_metric_to_csv(metrics, task_name, model_size, sparsity)
