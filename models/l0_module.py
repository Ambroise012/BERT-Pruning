...
class L0Module(Module):
    ...
    def initialize_whole_mlp(self):
        ...
        logger.info(f"Initialized whole mlps! Prunable_mode4l_size = {self.prunable_model_size}")
    ...
    # prend une valeur x, la normalise, calcule les logits, les ajuste avec la température et loga, 
    # applique une fonction d'activation (sigmoïde) et borne les valeurs pour obtenir la fonction de distribution cumulative pour la distribution concrète "étirée"
    ...
    def lagrangian_regularization(self, pruned_steps):
        ...
        # print(f"lambda_1: {self.lambda_1.item()}, lambda_2: {self.lambda_2.item()}")
        # Assuming lambda_1 and lambda_2 are PyTorch tensors, use .item() to get their values.
        lambda_1_value = self.lambda_1.item() if hasattr(self.lambda_1, 'item') else self.lambda_1
        lambda_2_value = self.lambda_2.item() if hasattr(self.lambda_2, 'item') else self.lambda_2

        # with open(f'constantes.csv', 'a', newline='') as csvfile:
        #     writer = csv.writer(csvfile)
        #     writer.writerow([lambda_1_value, lambda_2_value])
    ...
    def _deterministic_z(self, size, loga):
        ...
        if num_zeros > 0:
            if soft_mask.ndim == 0:
                ...
                # num_zeros lowest probability indices in soft_mask are identified using the torch.topk 
                # These indices correspond to the weights with the lowest probability of conservation
                # And are then set to zero in the mask
    ...
    # méthode forward pour obtenir les masques de pruning pendant l'entraînement ou l'inférence, en fonction du paramètre training
