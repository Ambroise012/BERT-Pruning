...
from models.modeling_bert_adapter import CoFiBertForSequenceClassification
from trainer.trainer_adapter import CoFiTrainer 
...
def main():
    ...
    teacher_model = None
    if additional_args.do_distill:
        teacher_model = CoFiBertForSequenceClassification.from_pretrained(
    ...
    model = CoFiBertForSequenceClassification.from_pretrained(
    ...
    ####################  END OF DISTILLATION ? #####################
    ####################  SAVE THE DSITLL MODEL  ###################
        # Save the distilled model
    # if additional_args.do_distill:
    #     distill_dir = os.path.join(training_args.output_dir, "distill_dir")
    #     if not os.path.exists(distill_dir):
    #         os.makedirs(distill_dir)
    #     logger.info(f"Saving the distill model")
    #     model.save_pretrained(distill_dir)
    ...
    if additional_args.pretrained_pruned_model is not None:
        ...
        model = load_model(additional_args.pretrained_pruned_model, CoFiBertForSequenceClassification, zs)
        print(f"Model Size after pruning: {calculate_parameters(model)}")
    ########  PRUNING / L0 Regularization  #########
    l0_module = None
    ...
    # save model 
    model_save = training_args.output_dir
    if training_args.do_train:
        trainer.train()
        trainer.save_model(model_save)
        tokenizer.save_pretrained(training_args.output_dir)
        model.save_pretrained(training_args.output_dir)
        # sauvegarde_model = os.path.join(training_args.output_dir, "sauvergarde_model")
        # logger.info(f"Saving the entire model: hopefully with pytorch_model")
        # model.save_pretrained(sauvegarde_model)

    zs = load_zs(model_save)
    model_class = CoFiBertForSequenceClassification
    # for compressed models
    if zs is None:
        model = model_class.from_pretrained(model_save)
    ...
    print(f"Model size after pruning : {model_size}")
    ...
    #         print(f"Model accuracy after pruning: {eval_results['eval_accuracy']}")
    #     else:
